...
def test_get_dag_path():
    ...
    dag = maya_fn.api.get_dag_path("persp")
...
def test_get_dag_path_errors_on_dg_node():
    ...
    with pytest.raises(TypeError):
        maya_fn.api.get_dag_path("time1")


def test_get_depend_node():
    node = maya_fn.api.get_object("time1")
...
def test_get_plug_errors():
    ...
    with pytest.raises(LookupError):
        maya_fn.api.get_plug("persp.foobar")

    with pytest.raises(TypeError):
        maya_fn.api.get_plug("persp")
